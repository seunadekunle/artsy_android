...
class ArtistRepository(private val api: ApiService) {
    ...
    suspend fun addFavourite(artistId: String) = api.addFavourite(com.example.asssignment_4.network.FavouriteRequest(artistId))
    suspend fun removeFavourite(artistId: String) = api.removeFavourite(com.example.asssignment_4.network.FavouriteRequest(artistId))
}
